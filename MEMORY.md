...
> Cross-session pointer: global memory `vistas-project.md`. Last updated 2026-06-30.

## ▶ RESUME (one-paragraph current state + next step)

**▶▶▶ RESUME — 2026-06-30 (FUZZY SEARCH shipped live; Fund-skill UX/metric pass + skill-engine REDESIGN greenlit; MAIN MISSION unchanged):**
**5 terminal fixes SHIPPED LIVE + backed up** (publish_terminal `--no-rebuild`, live `_pages` `a3fd8f155` 00:43, source `e604078`): **(1)** FM-shortlist 3-col board overflow → CSS `minmax(0,1fr)`+`min-width:0` on the grid items + compact font/pad + 1500px stack (the `1fr`=`minmax(auto,1fr)` won't-shrink trap); **(2)** peer-envelope batting/slug y-axis (`_fsEnvPlot`) now scales to the SIGNAL band (p25/p50/p75+fund+neutral, bat clamp [0,100]) not the wide min-max envelope, so it's not a flat smear; **(3)** GP "Indexed performance" **rebase-on-zoom** — new reusable `attachRebaseZoom` re-rebases each series to 100 at the LEFTMOST VISIBLE date on crop/zoom in Rebase mode (Absolute = Y-fit only); stores the common-start basis so successive zooms never compound; y2/volume untouched; **(4)** Screen **Breadth now follows the #106 flow-basis toggle** — per-basis buyer/seller headcounts emitted (`funds_flows.stock_active_flows` +`g_buyers/g_sellers`; `build_stock_series` +`na_/g_`), `screens.py` `fb["breadth"]={gross,price_adj,net_active}`, JS `screenApplyBasis` remaps `net_breadth`; **default Breadth = PRICE-ADJUSTED basis** (buyers/sellers counted on `flow`=price-stripped-but-inflow-kept), now documented in the screen note + FM caveat; **(5)** **Sizing edge → ANNUALISED %/yr** `(Π(1+rp)/Π(1+ew))^(1/yrs)−1` (scale-free; the cumulative gap grew with history length — KV's catch), Python `sizing_drag_cagr` ↔ JS `fsComputeWindow` **parity-locked** (probe 0 mismatches), suppressed "—" for sub-24mo funds (added to the null-out tuple — review caught it), display "Sizing edge (vs equal-wt /yr)". Verified: adversarial pre-publish review workflow **GO** (0 blockers/parity-mismatch) + `_pup_fundskill`/`_pup_screen` PASS + analytics parity 0 + Node runtime PASS. Stale-test fix: `_pup_fundskill.js` `fmOK` gate hard-coded `cols===2` → `===3` (stale since #114's 3-col board).
- **★ SKILL-ENGINE REDESIGN — GREENLIT, spec'd (`SKILL_ENGINE_REDESIGN.md`, 105KB, 4 grounded components A/B/C/D), NOT built.** KV's critique of the live verdict: 20.5% "skilled" / 49.9% "ahead-not-significant" / ~61% skilled-among-resolved = **too cheap** (gross, pre-factor-deflation, no FDR), and the NAV-IR·√years gate needs **15-25y** (NAV breadth ~1-3 bets/yr) so it's useless for 3-7y tenures + NFOs. FIX (first-principles, `t=IC·√(BR·years)`): measure skill at the **BET level** (holdings-rank IC + **trade-alpha** from `net_active`, breadth ~hundreds/yr → declares in ~1-2y), express as a **calibrated empirical-Bayes POSTERIOR** (estimate+credible-interval+P(skilled)+decile rank; category prior → honest early read for NFOs), honesty rails = **factor/sector deflation + net-of-fee + FDR (Barras-Scaillet-Wermers)**, NAV-IR kept as slow corroborator. **★ HONEST CEILING (most important):** current bet-level signals are WEAK (holding-rank IC split-half persistence **0.097**, reliability ~0.7%) → the Bayesian container is honest but **can't manufacture signal** — it delivers honesty + ranking + FDR-cut false-positives + an early ESTIMATE-with-wide-bars, **NOT magic 1-yr certainty**. Expected: 20.5%→**~3-7% skilled**. Data gaps: **PIT index constituent weights (W-HIST, highest-value, lacking)**, **TER/expense ratios (NONE → net-of-fee blocked)**, factor library (buildable ~1d, not built), manager-tenure DB (missing → scheme-level). **RISK: OOS test (D.2) may FAIL → ships as DESCRIPTIVE past-skill, not forward signal.** KV decision: **build AFTER publish, publish AFTER audit; sequence = rails-first** (biggest honest win, no data gaps) → trade-alpha → posterior → OOS gate. (Metric clarifications to KV this session, all correct: Breadth=price_adj headcount; bootstrap=circular BLOCK-resample-WITH-REPLACEMENT of MEAN active return (block=3, 2000×), p=%resamples mean>0 — NOT a reorder; slugging=arithmetic magnitude descriptor, NOT net-result (the net is geometric excess CAGR); verdict = a gated decision tree.)
- **PLAN/RENAME AUDIT — verified NOT a bug (no fix needed).** KV suspected ABSL Quant ("insufficient", 23mo) was a Direct/Regular NAV mismatch. VERIFIED: ABSL Quant = genuine ~2y NFO (Direct NAV `152684` also 23mo, matches holdings); funds_nav canonicalizes NAV on **Direct-Growth**. The 51 "truncation candidates" (old funds w/ ~12mo holdings) = **SEBI-2024 renames** (Bluechip→Large Cap…) where old+new scheme-NAME strings share ONE `navindia_code`; `funds_attribution.load_panel` **groups by navindia_code** (+`canonical_code`) → correctly stitches → full 158mo (manifest-verified: AXIS Large Cap 158mo, Canara LargeCap 158mo "skilled", etc.). My name-grouped scan was an artifact. **Verdict is HOLDINGS-based, not NAV** → NAV-plan matching can't affect it. Data model: holdings keyed by 5-digit navindia_code (Capitaline); NAV `_amfi_nav_panel` keyed by 6-digit AMFI code; scheme_master=568 Direct schemes; funds_bridge matches by holdings-symbol overlap (760 holdonly).
- **✅ FUZZY SEARCH — SHIPPED LIVE + backed up (2026-06-30).** All 3 search surfaces (universe `MultiSelect`, single-select `ComboBox` incl. the funds picker, and the ⌘K command palette) now route through ONE shared, typo-+acronym-tolerant scorer in `static/vistas.js` (pure display layer — **no analytics, no Python parity port**): `fuzzyNorm`/`fuzzyPrep`(memoized per item as `_fz`; `{norm,words,flat,acr}`)/`damerauLev`(3-row rotation + cap + early-exit; catches transpositions)/`fuzzyTokenScore`/`fuzzyScore`. **Logic:** tokenize the query, EVERY token must clear floor 0.5 against SOME field (AND semantics); each token scored best-of {whole-prefix 1.0 · exact-word 1.0 · word-prefix 0.95 · acronym-prefix 0.92 (so `absl`⊂"AdityaBirlaSunLife"=abslfcf) · **glued-compound** 0.9 (space-insensitive substring, `flat`, so `smallcap`="small cap") · substring 0.86/0.85 · edit-distance}; results sorted by score; empty query = full list (original order). **★ Adversarial review (4-lens workflow) caught a NO-GO regression** — short-token edit-tolerance leaked clearly-wrong names (`kota`→Tata/Bata, `bank`→Bata, `cit`→Coal India, all at the 0.5 floor); **FIXED** = (i) no edit-distance for tokens <4 chars (substring/prefix/acronym only), (ii) tighter `tol=floor((len-1)/4)+1` (len 4-7 → 1 edit not 2), (iii) glued-compound despaced-substring **restores** `hdfc smallcap` that (ii) broke. Plus hardening: `fuzzyNorm` `String()`-coerces (numeric name can't blank a picker), punctuation-only query → no-match. **Verified:** node unit test (8 acceptance + 5 leak-regression + gibberish/punctuation, all PASS) on the sliced shipped functions + headless `_pup_fuzzy.js` on the BUILT/inlined site (same cases in-browser + DOM-wiring no-throw + **0 runtime errors**) + Node smoke-test PASS. Live `_pages` published `--no-rebuild`, source backup `vistas-codebase` `eca16b7`. Regression harness kept: `_pup_fuzzy.js`. (Skipped, consciously: MultiSelect uncapped render = pre-existing not a regression; dead `_match` wrappers = harmless; exact-whole-vs-interior-word ranking = matches old behavior.)
- **★ IMMEDIATE NEXT — 2 queued KV requests (2026-06-30, SCOUTED, not yet built) + the skill engine:** (a) **display-name normalize** to plan-neutral (strip trailing plan tokens "- Regular (G)"/"- Direct - Growth" but PRESERVE category words like "Equity/Regular Savings"; algorithm = pop trailing plan-tokens until a content word) at `funds_attribution` manifest (L449)+rec scheme_name (L278)+`funds_bridge` holdonly (L76) — also feeds cleaner names into the now-live fuzzy index; (b) **NAV-attachment audit** — ~459 holdings schemes don't name-match a Direct NAV; audit which genuinely have one unbridged + wire (for the vs-real-NAV surface only — funds_nav, deck.py L789-792 writes `data/funds_nav/`, vistas.js L188 lazy-loads); then **build the skill-engine redesign** (`SKILL_ENGINE_REDESIGN.md`, rails-first; build-after-publish, publish-after-audit).
