...
**▶▶▶ RESUME — 2026-06-30 (Fund-skill UX/metric pass + skill-engine REDESIGN greenlit; MAIN MISSION unchanged):**
...
- **★ IMMEDIATE NEXT — 3 KV requests (2026-06-30, SCOUTED, not yet built):** (a) **display-name normalize** to plan-neutral (strip trailing plan tokens "- Regular (G)"/"- Direct - Growth" but PRESERVE category words like "Equity/Regular Savings"; algorithm = pop trailing plan-tokens until a content word) at `funds_attribution` manifest (L449)+rec scheme_name (L278)+`funds_bridge` holdonly (L76); (b) **NAV-attachment audit** — ~459 holdings schemes don't name-match a Direct NAV; audit which genuinely have one unbridged + wire (for the vs-real-NAV surface only — funds_nav, deck.py L789-792 writes `data/funds_nav/`, vistas.js L188 lazy-loads); (c) **★ FUZZY SEARCH** — current matcher is substring `.includes()` only (`MultiSelect._match` L598, `ComboBox._match` L1558, the cmdk palette) so "absl flexi" (acronym+word) and typos "koatq qiant"→kotak quant fail; need typo-tolerant fuzzy (token + Damerau-Levenshtein + acronym). **★ Build-ready spec = `FUZZY_SEARCH_SPEC.md`** (algorithm + wiring + 5 acceptance test cases + verify/publish). **NEXT ACTION (post-compact 2026-06-30): build the fuzzy search from that spec** → review → publish. Then build the skill-engine redesign.
